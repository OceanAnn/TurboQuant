...
class AscendModelSlimConfig(QuantizationConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str, tid2eid=None) -> Optional["QuantizeMethodBase"]:
        ...
        elif isinstance(layer, AttentionLayerBase) and hasattr(layer, "_tq_config"):
            from .methods.turboquant import AscendTurboQuantAttentionMethod

            logger.debug("Select AscendKVCacheMethod(TQ) for %s (layer=%s)", prefix, "AttentionLayerBase[TQ]")
            return AscendKVCacheMethod(AscendTurboQuantAttentionMethod(self.quant_description, prefix))
