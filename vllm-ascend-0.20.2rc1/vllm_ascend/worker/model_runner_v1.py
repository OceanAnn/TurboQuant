...
# Patch TurboQuantConfig.slot_size_aligned to use our paper-faithful layout
# (4-bit MSE for both K and V + fp16 norms). Must run at import time, before
# any kv_cache_spec is created via get_kv_cache_spec.
def _patch_tq_slot_size():
    try:
        ...
        return
    if not getattr(TurboQuantConfig, "_ascend_patched", False):
        def patched_slot_size_aligned(self):
            return compute_paper_slot_size(self.head_dim)
        TurboQuantConfig.slot_size_aligned = property(patched_slot_size_aligned)
        TurboQuantConfig._ascend_patched = True
...
class NPUModelRunner(GPUModelRunner):
        ...
        for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
            ...
            for idx in range(len(kv_cache_tensor.shared_by)):
                ...
                if (
                    ...
                    or isinstance(layer_kv_cache_spec[layer_name], TQFullAttentionSpec)
    ...
    def _apply_turboquant_class_surgery(self):
        """Force class surgery for TurboQuant attention layers.

        When --kv-cache-dtype turboquant_* is used without weight quantization,
        get_quant_method may not be called, so create_weights (which does the
        class surgery) is skipped. This method scans all attention layers and
        upgrades impl to AscendTurboQuantAttentionBackendImpl if _tq_config
        is present.
        """
        ...
        for module in self.model.modules():
            if hasattr(module, "_tq_config") and hasattr(module, "impl"):
                impl = module.impl
                if not isinstance(impl, AscendTurboQuantAttentionBackendImpl):
                    impl.__class__ = AscendTurboQuantAttentionBackendImpl
                    if not hasattr(module, "kv_cache_torch_dtype") or \
                       module.kv_cache_torch_dtype != torch.uint8:
                        module.kv_cache_torch_dtype = torch.uint8
    ...
    def _reshape_kv_cache_tensors(
        ...
        # Force TQ class surgery before using kv_cache (may have been skipped
        # if get_quant_method was not called for non-quantized models)
        self._apply_turboquant_class_surgery()
        layer_kv_cache_spec = self._get_layer_kv_cache_specs(kv_cache_config)
            ...
            for layer_name in group.layer_names:
                ...
                if self.use_compress and isinstance(current_kv_cache_spec, (MLAAttentionSpec, SlidingWindowMLASpec)):
                    ...
                    if self.use_sparse and "cache_only_layers" not in layer_name:
                        ...
                        assert raw_tensor.numel() % tq_page_size_bytes == 0
                        num_blocks = raw_tensor.numel() // tq_page_size_bytes
                        assert num_blocks >= kv_cache_config.num_blocks
                        kv_cache_shape = (
                            num_blocks,
                            current_kv_cache_spec.block_size,
                        ...
                        k_cache = raw_tensor.view(torch.uint8).view(kv_cache_shape)
                        kv_caches[layer_name] = k_cache
                        continue
                    else:
